...
export default class DatabaseManager {
    ...
    updateRecord(tableName, primaryKey, primaryKeyValue, newData) {
        try {
            ...
            if (!newData || Object.keys(newData).length === 0) {
                console.log(`لا يمكن تحديث السجل في الجدول ${tableName}، حيث أن البيانات الجديدة غير محددة أو فارغة. يُرجى تقديم بيانات جديدة صالحة في شكل كائن.`);
                return false;
            }

            // التحقق من وجود الجدول
            if (!this.tableExists(tableName)) {
                console.error(`الجدول ${tableName} غير موجود.`);
                return;
            }
            ...
            if (!columns.includes(primaryKey)) {
                console.error(`العمود الرئيسي ${primaryKey} غير موجود في جدول ${tableName}.`);
                return;
            }
            ...
            console.error(`حدث خطأ أثناء تحديث السجل في جدول ${tableName}:`, error);
            throw error;
        }
    }
    ...
    deleteRecord(tableName, primaryKey, primaryKeyValue) {
        try {
            ...
            statement.run(primaryKeyValue);
            console.log(`تم حذف السجل من جدول ${tableName} بنجاح.`);
        } catch (error) {
            console.error(`حدث خطأ أثناء حذف السجل من جدول ${tableName}:`, error);
            throw error;
        }
    }
    ...
    addNewColumn(tableName, columnName, columnType) {
        try {
            ...
            if (columns.includes(columnName)) {
                console.error(`العمود ${columnName} موجود بالفعل في جدول ${tableName}.`);
                return;
            }

            // إنشاء العمود الجديد
            this.db.exec(`ALTER TABLE ${tableName} ADD COLUMN ${columnName} ${columnType}`);
            console.log(`تمت إضافة العمود ${columnName} إلى جدول ${tableName}.`);
        } catch (error) {
            console.error(`حدث خطأ أثناء إضافة العمود ${columnName} إلى جدول ${tableName}:`, error);
            throw error;
        }
    }
    ...
    deleteColumn(tableName, columnName) {
        try {
            ...
            const columns = this.getColumnNames(tableName);
            if (!columns.includes(columnName)) {
                console.error(`العمود ${columnName} غير موجود في جدول ${tableName}.`);
                return;
            }

            // حذف العمود
            this.db.exec(`ALTER TABLE ${tableName} DROP COLUMN ${columnName}`);
            console.log(`تم حذف العمود ${columnName} من جدول ${tableName}.`);
        } catch (error) {
    ...
    deleteTable(tableName) {
        try {
            ...
            console.log(`تم حذف الجدول ${tableName} من قاعدة البيانات.`);
        } catch (error) {
            console.error(`حدث خطأ أثناء حذف الجدول ${tableName}:`, error);
            throw error;
        }
    }
    ...
    findRecordAll(tableName, columnName, columnValue) {
        try {
            ...
            console.log(`السجلات في جدول ${tableName} حيث ${columnName} تساوي ${columnValue}:`, records?.length || 0);
            return records;
        } catch (error) {
            console.error(`حدث خطأ أثناء جلب السجلات من جدول ${tableName} حيث ${columnName} تساوي ${columnValue}:`, error);
            throw error;
        }
    }
